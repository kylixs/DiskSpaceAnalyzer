...
1. WHEN 用户启动应用程序 THEN 系统 SHALL 显示主窗口包含磁盘选择界面
2. WHEN 用户选择磁盘分区 THEN 系统 SHALL 开始异步扫描并在GUI中显示进度条和状态信息
3. WHEN 扫描过程进行中 THEN 系统 SHALL 每100ms更新一次界面显示，包括扫描进度、当前路径和已发现的文件数量
4. WHEN 扫描开始 THEN 目录树 SHALL 立即显示正确的根目录，不再空白
5. WHEN 扫描过程中 THEN 目录树 SHALL 实时显示新发现的目录结构，根目录路径与用户选择一致
6. WHEN 用户点击取消扫描 THEN 系统 SHALL 立即停止所有扫描异步任务并保留已扫描结果
7. WHEN 扫描遇到权限问题 THEN 系统 SHALL 在状态栏显示错误信息而不是弹框，继续扫描其他区域
8. WHEN 扫描文件系统 THEN 系统 SHALL 自动过滤0KB文件、符号链接和重复的硬链接文件
